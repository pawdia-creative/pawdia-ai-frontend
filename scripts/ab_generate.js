#!/usr/bin/env node
/**
 * Batch A/B generator script
 * Usage: node scripts/ab_generate.js <ADMIN_TOKEN>
 *
 * This will read originals from public/examples/{style}/original.jpg
 * and create baseline vs enhanced generations (3 each) via /api/generate.
 */
...
if (!TOKEN) {
  console.error('Usage: node scripts/ab_generate.js <ADMIN_TOKEN>');
  process.exit(1);
}

const OUT_DIR = path.resolve(__dirname, '..', 'public', 'temp-samples');
...
function strongPrompt(styleName) {
  // stronger, reference masters where appropriate
  switch (styleName) {
    case 'Oil Painting':
...
async function run() {
  ...
  for (const style of styles) {
    const origPath = path.resolve(__dirname, '..', 'public', 'examples', style.id, 'original.jpg');
    try {
      await fs.access(origPath);
    } catch (e) {
      console.warn(`Original not found for style ${style.id} at ${origPath}, skipping.`);
      continue;
    }
      ...
      for (let i=1;i<=3;i++) {
        ...
        try {
          ...
          if (!resp.ok) {
            ...
            // if provider quota exhausted, stop further runs
            if (resp.status === 403) {
          ...
          if (data.base64 || data.b64_json || (data.raw && data.raw.base64)) {
            ...
            // try to find nested base64
            const rawStr = JSON.stringify(data.raw);
            // fallback: try to locate a data URI in the raw JSON string
