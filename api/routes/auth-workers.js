...
const generateToken = (user) => {
  return jwt.sign({ 
    ...
    email: user.email
  }, globalThis.env?.JWT_SECRET || process.env.JWT_SECRET, { expiresIn: '7d' });
};
...
router.get('/verify-email', async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: '邮箱验证过程中发生错误' });
  }
});
...
router.post('/resend-verification', auth, async (req, res) => {
  try {
    ...
    if (user.isVerified) {
      return res.status(400).json({ message: '用户已经验证过邮箱' });
    }
    ...
    return res.json({ 
      message: emailSent ? '验证邮件已重新发送' : '验证邮件发送失败，请联系客服',
      emailSent
    });

  } catch (error) {
    console.error('Resend verification error:', error);
    return res.status(500).json({ message: '重新发送验证邮件时发生错误' });
  }
});
